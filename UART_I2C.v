...
reg  [3:0] state;
reg  [7:0] memory [0:1]; // taking 12 bits hence 2 depth array
reg  [1:0] memory_counter;
reg        start_i2c ; 
wire       i2c_done;
reg  [15:0] dac_data;
...
ila_1 uut2 ( clk , dbg_sda_listener , dbg_start , dac_data , dbg_state1);
always@(posedge clk)
begin
    if(~rst)
    begin
    ...
    begin
        case(state)
            ...
            begin
                ...
                else if(memory_counter == 2)
                begin
                    dac_data <= {memory[0] , memory[1]};
                    memory_counter  <= 0;
                    state <= RX_STOP;
                end
            end
